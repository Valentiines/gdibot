...
var botConfigs = {
    ...
    ticketsystem: {
    "ticketCategoryID": "631504984466259977",
    "createTicketChannelID": "561362794386030602"
}
};
...
client.on("message", async function (message) {
    ...
    if (command === "ticket" && botConfigs.plugins[12].activated == true) {
        if (botConfigs.ticketsystem.createTicketChannelID == "" || botConfigs.ticketsystem.createTicketChannelID == null || botConfigs.ticketsystem.createTicketChannelID == undefined || botConfigs.ticketsystem.ticketCategoryID == "" || botConfigs.ticketsystem.ticketCategoryID == null || botConfigs.ticketsystem.ticketCategoryID == undefined) return message.channel.send("The 		system is not working - Please run the config").catch(console.error);
        if (message.channel.id === botConfigs.ticketsystem.createTicketChannelID) {
            ...
            if (subject.length > 60) return message.channel.send("Subject max length: 60 characters").catch(console.error);
            message.guild.createChannel(message.author.id, 'text').then(async m => {
				...
				m.send(`<@${message.author.id}> Thanks for submitting a ticket!`);
				m.send("**Your report subject is:** " + subject, {files: ["https://i.imgur.com/KxugVHS.png"]});
            });
            message.author.send("Attention! Ticket created! We appreciate you contacting us. Please make sure to @ the staff.");
            message.delete().catch(O_o => {}); 
        } else {
    ...
    if (command === "close") {
        if (!args[0]) return message.channel.send("Please specify what you want close - Example: !gdi close ticket").catch(console.error);
        if (args[0] === "ticket") {
            if (message.channel.parent.id === botConfigs.ticketsystem.ticketCategoryID) {
                message.channel.delete();
                if (message.channel.name === message.author.id) {
                    message.author.send("The ticket has been closed and deleted, thankyou for making the ticket. \n\If you have anymore troubles, don't be shy! **!gdi ticket subject**");
                } else {
                    message.author.send("The ticket has been closed and deleted, thankyou for making the ticket. \n\If you have anymore troubles, don't be shy! **!gdi ticket subject**");
                    client.fetchUser(message.channel.name)
                        .then(user => {
                            user.send("The ticket has been closed and deleted by an administrator. **Thankyou!**")
                        })
                }
            } else {
